 ...
 * sub-condition(s) instead of the whole (often large) possible/potential
 * block. Mirrors backend/audit_build_rules.js - keep both in sync.
 */
export function getFailingConditions(node: PredicateNode | null | undefined, ctx: BuildContext): string[] {
  ...
  if (node.all) {
    return node.all
      ...
      .flatMap(child => (child.all ? getFailingConditions(child, ctx) : [describePredicate(child)]));
  }
  return evaluatePredicate(node, ctx) ? [] : [describePredicate(node)];
}
